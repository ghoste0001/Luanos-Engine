...
static float GIZMO_ROTATE_SPEED = 1.0f;
static bool  GIZMO_ROTATE_SNAP_ENABLED = true;
static float GIZMO_ROTATE_SNAP_DEG = 15.0f;
static bool  GIZMO_ROTATING = false;
static float GIZMO_ROTATE_ACCUM = 0.0f;
static float GIZMO_ROTATE_ACCUM_APPLIED = 0.0f;

void SetGizmoRotateSpeed(float speed) { GIZMO_ROTATE_SPEED = speed; }
float GetGizmoRotateSpeed(void) { return GIZMO_ROTATE_SPEED; }

void SetGizmoRotateSnapEnabled(bool enabled) { GIZMO_ROTATE_SNAP_ENABLED = enabled; }
bool GetGizmoRotateSnapEnabled(void) { return GIZMO_ROTATE_SNAP_ENABLED; }

void SetGizmoRotateSnapDegrees(float degrees) { GIZMO_ROTATE_SNAP_DEG = degrees; }
float GetGizmoRotateSnapDegrees(void) { return GIZMO_ROTATE_SNAP_DEG; }

static float SnapAngleRad(float angleRad, float stepDeg)
{
    if (stepDeg <= 0.0f) return angleRad;
    const float step = stepDeg * (PI / 180.0f);
    return roundf(angleRad / step) * step;
}
...
static GizmoGlobals GIZMO = {
	...
	.trArrowLengthFactor = 0.15f, // changed (was above width when in GizmoGlobals its below, causing a compilation error)
...
{
	// changed (the order of the values cause scale was at the top causing a compilation error)
	return (Transform){
		.translation = {0},
	...
	if (action != GZ_ACTION_NONE)
	{
		...
		else
		{
			...
			case GZ_ACTION_ROTATE:
			{
				...
				{
					GIZMO_ROTATING = true;
					GIZMO_ROTATE_ACCUM = 0.0f;
					GIZMO_ROTATE_ACCUM_APPLIED = 0.0f;
				}

				GIZMO.activeTransform->rotation = GIZMO.startTransform.rotation;

				float deltaRaw = Clamp(
					Vector3DotProduct(pVec, Vector3Add(data->right, data->up)),
					-2 * PI, +2 * PI
				);

				// Adjust delta (too fast otherwise lol)
				deltaRaw *= 0.1f * GIZMO_ROTATE_SPEED;

				// Accumulate raw delta
				GIZMO_ROTATE_ACCUM += deltaRaw;

				float accumWanted = GIZMO_ROTATE_ACCUM;
				if (GIZMO_ROTATE_SNAP_ENABLED)
					accumWanted = SnapAngleRad(accumWanted, GIZMO_ROTATE_SNAP_DEG);

				const float deltaApply = accumWanted - GIZMO_ROTATE_ACCUM_APPLIED;
				GIZMO_ROTATE_ACCUM_APPLIED = accumWanted;

				if (GIZMO.activeAxis & GZ_ACTIVE_X)
				{
					const Quaternion q = QuaternionFromAxisAngle(data->axis[GZ_AXIS_X], deltaApply);
					GIZMO.activeTransform->rotation = QuaternionMultiply(q, GIZMO.activeTransform->rotation);
				}
				if (GIZMO.activeAxis & GZ_ACTIVE_Y)
				{
					const Quaternion q = QuaternionFromAxisAngle(data->axis[GZ_AXIS_Y], deltaApply);
					GIZMO.activeTransform->rotation = QuaternionMultiply(q, GIZMO.activeTransform->rotation);
				}
				if (GIZMO.activeAxis & GZ_ACTIVE_Z)
				{
					const Quaternion q = QuaternionFromAxisAngle(data->axis[GZ_AXIS_Z], deltaApply);
					GIZMO.activeTransform->rotation = QuaternionMultiply(q, GIZMO.activeTransform->rotation);
				}
